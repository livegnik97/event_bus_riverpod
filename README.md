...
Every call to `emit()` / `emitAsync()` automatically generates a `BusMetadata` object with a precise `timestamp`. The emitter can optionally attach a `source` string and arbitrary `extraData` to carry extra context all the way to the listeners.

**Two types:**

| Type | Purpose | Who creates it |
|------|---------|---------------|
| `BusMetadataForEmit` | Passed to `emit()` / `emitAsync()` by the caller | The emitter |
| `BusMetadata` | Received by `*WithMeta` listeners; auto-generated by the bus | The bus (inherits from `BusMetadataForEmit`) |
...
// With source
ref.event(onUserLogin).emit(user, metadata: BusMetadataForEmit(source: 'login_screen'));
...
await ref.event(onUserLogin).emitAsync(user, metadata: BusMetadataForEmit(
  source: 'login_screen',
));
```
...
```dart
ref.event(onAddToCart).emit(item, metadata: BusMetadataForEmit(
...
```dart
ref.event(onUserLogin).emit(user, metadata: BusMetadataForEmit(source: 'onboarding'));
...
| Method | `BusMetadataForEmit?` on emit |
|--------|------------------------------|
